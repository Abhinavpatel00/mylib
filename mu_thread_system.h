...
MU_TS_INLINE void mu_ts_memorybarrier_acquire(void) { _ReadWriteBarrier(); }
MU_TS_INLINE void mu_ts_memorybarrier_release(void) { _ReadWriteBarrier(); }

MU_TS_INLINE uint32_t mu_ts_atomic32_load_relaxed(mu_ts_atomic32_t* p) { return *p; }
...
MU_TS_INLINE uint64_t mu_ts_atomic64_load_relaxed(mu_ts_atomic64_t* p) { return *p; }
...
MU_TS_INLINE void mu_ts_memorybarrier_acquire(void) { __asm__ __volatile__("" : : : "memory"); }
MU_TS_INLINE void mu_ts_memorybarrier_release(void) { __asm__ __volatile__("" : : : "memory"); }
...
MU_TS_INLINE void mu_ts_mutex_exit(mu_ts_mutex* m) { DeleteCriticalSection(m); }
MU_TS_INLINE void mu_ts_mutex_lock(mu_ts_mutex* m) { EnterCriticalSection(m); }
MU_TS_INLINE void mu_ts_mutex_unlock(mu_ts_mutex* m) { LeaveCriticalSection(m); }
...
MU_TS_INLINE void mu_ts_cond_exit(mu_ts_cond* c) { (void)c; }
MU_TS_INLINE void mu_ts_cond_wake_one(mu_ts_cond* c) { WakeConditionVariable(c); }
MU_TS_INLINE void mu_ts_cond_wake_all(mu_ts_cond* c) { WakeAllConditionVariable(c); }
...
MU_TS_INLINE void mu_ts_thread_detach(mu_ts_thread_handle h) { CloseHandle(h); }
...
MU_TS_INLINE bool mu_ts_mutex_init(mu_ts_mutex* m) { return pthread_mutex_init(m, NULL) == 0; }
MU_TS_INLINE void mu_ts_mutex_exit(mu_ts_mutex* m) { pthread_mutex_destroy(m); }
MU_TS_INLINE void mu_ts_mutex_lock(mu_ts_mutex* m) { pthread_mutex_lock(m); }
MU_TS_INLINE void mu_ts_mutex_unlock(mu_ts_mutex* m) { pthread_mutex_unlock(m); }

MU_TS_INLINE bool mu_ts_cond_init(mu_ts_cond* c) { return pthread_cond_init(c, NULL) == 0; }
MU_TS_INLINE void mu_ts_cond_exit(mu_ts_cond* c) { pthread_cond_destroy(c); }
MU_TS_INLINE void mu_ts_cond_wake_one(mu_ts_cond* c) { pthread_cond_signal(c); }
MU_TS_INLINE void mu_ts_cond_wake_all(mu_ts_cond* c) { pthread_cond_broadcast(c); }
...
MU_TS_INLINE void mu_ts_thread_join(mu_ts_thread_handle h) { pthread_join(h, NULL); }
MU_TS_INLINE void mu_ts_thread_detach(mu_ts_thread_handle h) { pthread_detach(h); }
MU_TS_INLINE uint64_t mu_ts_cpu_count(void)
{
...
MU_TS_INLINE bool mu_ts__ensure_task_capacity(mu_ts_system* t, uint64_t need)
{
    ...
    if (need <= t->tasks_cap) return true;
...
MU_TS_INLINE bool mu_ts_init(mu_ts_handle* out, const mu_ts_init_desc* in_desc)
{
    ...
    *out = NULL;
    ...
    if (count > cpu_count) count = cpu_count;
    ...
    if (mutex_ok) tasks_ok = mu_ts_cond_init(&t->condition_tasks);
    if (mutex_ok && tasks_ok) idle_ok = mu_ts_cond_init(&t->condition_idle);

    if (!mutex_ok || !tasks_ok || !idle_ok) {
        if (idle_ok) mu_ts_cond_exit(&t->condition_idle);
        if (tasks_ok) mu_ts_cond_exit(&t->condition_tasks);
        if (mutex_ok) mu_ts_mutex_exit(&t->mutex);
        MU_TS_FREE(t->threads);
        MU_TS_FREE(t->boot);
        MU_TS_FREE(t);
        return false;
...
MU_TS_INLINE void mu_ts_exit(mu_ts_handle* h, const mu_ts_exit_desc* in_desc)
{
    ...
    *h = NULL;
    ...
    mu_ts_mutex_lock(&t->mutex);
    t->stop = true;
    if (desc->abandon_tasks) t->stop_abandon = true;
    mu_ts_cond_wake_all(&t->condition_tasks);
    mu_ts_mutex_unlock(&t->mutex);

    for (uint64_t i = 0; i < t->thread_count; ++i) {
        if (!desc->detach_threads) mu_ts_thread_join(t->threads[i]);
        else mu_ts_thread_detach(t->threads[i]);
    }
...
MU_TS_INLINE void mu_ts_add_tasks(mu_ts_handle h, mu_ts_task_fn fn, uint64_t count, uint64_t user_size, void* user_array)
{
    ...
    if (count == 1) mu_ts_cond_wake_one(&t->condition_tasks);
    else mu_ts_cond_wake_all(&t->condition_tasks);
